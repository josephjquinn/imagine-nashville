...
interface EducationPriority {
  priority: string;
  questionId: string;
  label: string;
}

const EDUCATION_PRIORITIES: EducationPriority[] = [
  {
    questionId: "Q660_A",
    priority:
  ...
  {
    questionId: "Q660_C",
    priority: "Increase school safety",
    label: "School Safety",
  },
  {
    questionId: "Q660_D",
...
export const EducationPrioritiesTable: React.FC<
  EducationPrioritiesTableProps
> = ({ data }) => {
  const processedData = useMemo(() => {
    ...
    EDUCATION_PRIORITIES.forEach((priority) => {
    ...
    Object.values(NEIGHBORHOOD_CODE_MAP).forEach((neighborhood) => {
      const priorityScores = EDUCATION_PRIORITIES.map((priority) => ({
    ...
    <div className="overflow-x-auto">
      <table className="min-w-full divide-y divide-gray-200">
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {EDUCATION_PRIORITIES.map((priority, index) => (
            <tr
