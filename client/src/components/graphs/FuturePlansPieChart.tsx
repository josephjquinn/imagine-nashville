...
interface FuturePlansPieChartProps {
  data: SurveyResponse[];
  title?: string;
}

export const FuturePlansPieChart: React.FC<FuturePlansPieChartProps> = ({
  data,
  title,
}) => {
  const field = "Q105";
    ...
    return (
      <div className="flex items-center justify-center h-full">
        <p className="text-gray-500">
          No valid data available for Future Plans Distribution
        </p>
      </div>
    );
  }
  ...
  const option: EChartsOption = {
    title: {
      text: title || "Future Plans Distribution",
      left: "center",
    },
    tooltip: {
      trigger: "item",
